...
- Read the per-share CSP even when raw_csp is unset (#71)

[0.7.3]: https://github.com/ernolf/files_sharing_raw/releases/tag/v0.7.3
...
- Admin setup check (Settings → Administration → Overview): warns when
  `files_sharing_raw` is not yet registered in Nextcloud core's `rootUrlApps`
  list, with a direct link to the documentation.

### Changed
- `patch-route-parser.sh`: now auto-detects the Nextcloud root by walking up
  the directory tree until `occ` is found; prompts interactively if
  auto-detection fails.
- Built JS assets (`js/`) removed from git tracking; generated by the build
  process and excluded via `.gitignore`.
